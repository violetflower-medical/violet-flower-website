...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      console.log('DEBUG: Fetching clients from Supabase...');
      ...
      if (error) {
        ...
        console.log('DEBUG: Clients fetched successfully:', data?.length, 'items');
        setClients(data || []);
      }
    };
        ...
        <div className="container mx-auto px-4 md:px-8">
          ...
          {clients.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 opacity-40">
              <div className="w-16 h-1 bg-border rounded-full mb-4" />
              <p className="text-sm font-bold uppercase tracking-widest text-muted-foreground italic">
                {locale === 'en' ? 'Partnership data syncing...' : 'جاري مزامنة بيانات الشركاء...'}
              </p>
            </div>
          )}
        </div>
